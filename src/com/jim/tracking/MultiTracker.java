 ...
 *
 * TODO
 * - Somehow prefer newer tracks over older tracks.
 *   Hard to implement because it mustn't interfere with the maxJump parameter.
 */
public class MultiTracker implements MotionDetector.Filter {
    ...
    private List<TrackWithEllipse> trackMulti(List<MotionDetector.DetectedObject> detectedObjects, List<TrackWithEllipse> tracks, Params params, boolean allowNewTracks, Size imageSize, int frameNumber) {
        ...
        int[] objActions;
        if (numObjects > 0) {
            int[][] assignments = assignTracksAndObjects(distances, objDistances, params.trParams.maxJump, params.trParams.minGap);
        ...
        if (allowNewTracks) {
            for (int objIdx : objActions) {
                        ...
                        TrackWithEllipse track = new TrackWithEllipse(nextObjectId++, cfg, obj.centroid, obj.ellipse);
...
//        System.out.println(numObjects + " objects -> tracks: " + Util.join(result, ", "));
     ...
     */
    private int[][] assignTracksAndObjects(double[][] distances, double[][] objectDistances, double maxJump, double minGap) {
        ...
        simpleAssignments(distances, maxJump, assignedTrackToObject, assignedObjectToTrack, bestTrackToObject, bestObjectToTrack);

        // Now solve problems
        // Look for objects which are artifacts of the process - i.e. very close together, and discard them
...
//                System.out.println("objDist = " + objDist + ", " + obj + "-" + bestObj);
        ...
        while (simpleAssignments(distances, maxJump, assignedTrackToObject, assignedObjectToTrack, bestTrackToObject, bestObjectToTrack) > 0) {
        }
...
//        }
    ...
    private int simpleAssignments(double[][] distances, double maxJump, int[] assignedTrackToObject, int[] assignedObjectToTrack, int[] bestTrackToObject, int[] bestObjectToTrack) {
        int successfulAssignments = 0;

        // Get best objects for each track
        for (int i = 0; i < bestTrackToObject.length; i++)
            bestTrackToObject[i] = minIndexInCol(distances, i);
        // Get best tracks for each object
        for (int i = 0; i < bestObjectToTrack.length; i++)
            bestObjectToTrack[i] = minIndexInRow(distances, i);
...
//        System.out.println("Objects to tracks: " + Util.join(assignedObjectToTrack, ", "));
    ...
    private int minIndexInCol(double[][] distances, int col) {
        int minRow = -1;
    ...
    private int minIndexInRow(double[][] distances, int row) {
        int minCol = -1;
        double minValue = Double.MAX_VALUE;
